...
class Kapucijnen(Card):
    def __init__(self):
        self._key = "kapucijnen"
            ...
            <div class="item{% if oldNews %} oldNews{% endif %}">
                <div class="card contemplation mp3">
                    ...
                    <div class="actions">
                        ...
                        {% set title = my_encode("Kapucijnen: " + data['title'] + ' via alledaags.gelovenleren.net') %}
                        {% set short_title = my_encode("Kapucijnen: " + data['title']) %}
    ...
    def harvestSync(self):
        # load from feed
        feed = "https://kapucijnen.be/category/audios/feed/"
        harvest = getRSS(feed)
        data = {
            'name': "Kapucijnen",
            'image': "https://alledaags.gelovenleren.net/var/kapucijnen.jpg",
            'key': self._key
        }
        try:
            item = harvest['item'][0]
