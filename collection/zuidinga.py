...
class Zuidinga(Card):
            ...
            <div class="item{% if oldNews %} oldNews{% endif %}">
                <div class="card contemplation">
                    ...
                    <div class="actions">
                        ...
                        {% set title = my_encode("Banneuxblog: " + data['title'] + ' via alledaags.gelovenleren.net') %}
                        {% set short_title = my_encode("Banneuxblog: " + data['title']) %}
    ...
    def harvestSync(self):
        # load from feed
        feed = "https://zuidinga.wordpress.com/feed/"
        harvest = getRSS(feed)
        data = {
            'name': "Banneuxblog",
            'image': "/static/banneux.png",
            'key': self._key
        }
        try:
            item = harvest['item'][0]
